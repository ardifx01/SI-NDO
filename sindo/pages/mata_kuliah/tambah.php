...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($kode_mk) || empty($nama_mk) || empty($sks) || empty($dosen)) {
        ...
        // Cek kode MK sudah ada
        $stmt = $pdo->prepare("SELECT id FROM mata_kuliah WHERE kode_mk = ? AND user_id = ?");
        $stmt->execute([$kode_mk, $_SESSION['user_id']]);
        if ($stmt->fetch()) {
            $error = 'Kode mata kuliah sudah digunakan!';
        } else {
            $pdo->beginTransaction();
            try {
                $stmt = $pdo->prepare("INSERT INTO mata_kuliah (kode_mk, nama_mk, sks, dosen, ruangan, user_id) VALUES (?, ?, ?, ?, ?, ?)");
                if ($stmt->execute([$kode_mk, $nama_mk, $sks, $dosen, $ruangan, $_SESSION['user_id']])) {
                    ...
                    if (isset($_POST['hari']) && is_array($_POST['hari'])) {
                        foreach ($_POST['hari'] as $index => $hari) {
                            if (!empty($hari) && !empty($_POST['jam_mulai'][$index]) && !empty($_POST['jam_selesai'][$index])) {
                                ...
                                $stmt->execute([$mk_id, $hari, $_POST['jam_mulai'][$index], $_POST['jam_selesai'][$index]]);
                            }
                        }
                    }
    ...
    <div class="card shadow-lg">
        <div class="card-header bg-primary text-white">
            <div class="d-flex justify-content-between align-items-center">
                <h2 class="mb-0"><i class="bi bi-book me-2"></i>Tambah Mata Kuliah Baru</h2>
            ...
            <form method="post" id="form-matkul">
                    ...
                    <div class="col-md-5">
                        <div class="form-floating">
                            <input type="text" class="form-control" id="dosen" name="dosen" 
                                   placeholder="Dr. John Doe, M.Kom" required>
                ...
                <div id="jadwal-container">
                    <div class="row g-3 mb-3 jadwal-item">
                        ...
                        <div class="col-md-3">
                            <input type="time" class="form-control" name="jam_mulai[]" aria-label="Jam mulai">
                        </div>
                        <div class="col-md-3">
                            <input type="time" class="form-control" name="jam_selesai[]" aria-label="Jam selesai">
                        </div>
